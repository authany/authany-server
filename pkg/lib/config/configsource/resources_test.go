...
`),
	...
	Convey("AuthgearYAML fraud_protection feature config", t, func() {
		path := "authgear.yaml"
		app := resource.LeveledAferoFs{FsLevel: resource.FsLevelApp}
		descriptor := &AuthgearYAMLDescriptor{}

		Convey("is_modifiable=false blocks changes to fraud_protection", func() {
			ctrl := gomock.NewController(t)
			defer ctrl.Finish()
...
`),
			...
			So(err, ShouldBeError, `invalid authgear.yaml:
/fraud_protection: fraud_protection config is not modifiable`)
		})

		Convey("is_modifiable=false allows saving unchanged fraud_protection", func() {
			ctrl := gomock.NewController(t)
			defer ctrl.Finish()
...
`),
		...
		Convey("is_modifiable=true allows changes to fraud_protection", func() {
			ctrl := gomock.NewController(t)
			defer ctrl.Finish()
			...
			featureConfig, _ := config.ParseFeatureConfig(ctx, []byte(`
fraud_protection:
  is_modifiable: true
`))
			ctx := context.Background()

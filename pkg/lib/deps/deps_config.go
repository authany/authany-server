...
var ConfigDeps = wire.NewSet(
	...
	wire.FieldsOf(new(*config.ProofOfPhoneNumberVerificationConfig),
		"Hook",
	),
...
var secretDeps = wire.NewSet(
	...
	ProvideWhatsappCloudAPICredentials,
...
func ProvideTencentCredentials(c *config.SecretConfig) *config.TencentCredentials {
	s, _ := c.LookupData(config.TencentCredentialsKey).(*config.TencentCredentials)
	return s
}
...
func ProvideWhatsappCloudAPICredentials(c *config.SecretConfig) *config.WhatsappCloudAPICredentials {
	s, _ := c.LookupData(config.WhatsappCloudAPICredentialsKey).(*config.WhatsappCloudAPICredentials)
	return s
}
